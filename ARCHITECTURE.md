...
## 39. Knowledge progression and corruption (Stage 39)

Пользовательская research-система удалена. `FactionState.resources.specimens` и `FactionState.specimensCollected` временно сохранены как legacy-compatible сериализуемые поля, но в актуальном gameplay оба синхронно представляют **Познание Орсии**. Новые механики не должны трактовать их как расходуемые образцы. Максимум и пороги задаёт `data/campaign/knowledgeRules.ts`.

`core/events/LocationEvent.ts` выдаёт `knowledge`, а `core/artifacts/acquireArtifact.ts` даёт +1 Познания за новый артефакт и материализует его слабую цену: детерминированно усиливает небольшую часть вражеских городских гарнизонов. `Math.random()` для этого запрещён. Новые посещения городов артефактов не дают.

Два визуальных порога Познания (25% и 75%) вычисляются в UI из data-rule и только меняют presentation. Они не должны влиять на simulation. Боевые blood/shock/dramatic эффекты также вычисляются детерминированно из BattleTimeline и не являются источником боевого результата.

Save v20→v21 конвертирует старую научную экономику в монотонное Познание и удаляет ранее материализованные research-traits игрока.
